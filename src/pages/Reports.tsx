...
import { PDFExportService } from "@/services/reports/pdfExportService";
import { toast } from "sonner";
import { 
  TrendingUp, FileText, Target, Activity, Zap, 
  BarChart, DollarSign, Users, Download
} from "lucide-react";
...
const Reports = () => {
  const handleExportToPDF = async () => {
    ...
    try {
      const reportData = {
        title: "ChariotAI Marketing Report",
        ...
        metrics: {
          ...
          roas: 3.0,
          conversions: 217,
          improvement: "18% improvement vs last week"
        },
        channelData: [
          { platform: "Google Ads", spend: 2450, roas: 3.8, conversions: 127 },
          { platform: "Facebook", spend: 1850, roas: 2.4, conversions: 73 },
          { platform: "Instagram", spend: 1100, roas: 2.9, conversions: 42 }
        ],
        audienceData: {
          demographics: {
            ageDistribution: { "18-24": 35, "25-34": 42, "35+": 23 },
            genderDistribution: { male: 45, female: 55 }
          },
          topLocations: [
            { location: "New York", percentage: 85 },
            { location: "Los Angeles", percentage: 65 },
            { location: "Chicago", percentage: 45 }
          ]
        }
      };

      await PDFExportService.exportReportToPDF(reportData);
  ...
  return (
    <DashboardLayout className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleExportToPDF} className="bg-primary hover:bg-primary/90">
