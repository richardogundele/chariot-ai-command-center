...
export const DashboardLayout = ({ children, className = "", headerActions, headerLeftText }: DashboardLayoutProps) => {
        ...
        <header className={`fixed top-0 right-0 ${getHeaderLeftPosition()} z-40 bg-background/95 backdrop-blur-xl border-b border-border px-4 sm:px-6 py-3 sm:py-4 transition-all duration-300 shadow-lg pointer-events-none`}>
          <div className="flex items-center justify-between gap-2 sm:gap-4 pointer-events-auto">
            <div className="min-w-0">
              {headerLeftText && (
                <span className="text-sm sm:text-base font-medium text-foreground/90 truncate block">
                  {headerLeftText}
                </span>
              )}
            </div>
